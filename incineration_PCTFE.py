...
# LCA unit factors used in PCTFE block
...
mass_fraction_PCTFE, correlation_distributions, correlation_parameters = lhs.lhs_distribution(design_assumptions.loc['mass_fraction_PCTFE'], correlation_distributions, correlation_parameters, n_samples)

rho_pctfe, correlation_distributions, correlation_parameters = lhs.lhs_distribution(design_assumptions.loc['rho_pctfe'], correlation_distributions, correlation_parameters, n_samples)
...
# PCTFE specific stack emissions
CO2_to_air_incin_PCTFE, correlation_distributions, correlation_parameters = lhs.lhs_distribution(design_assumptions.loc['CO2_to_air_incin_PCTFE'], correlation_distributions, correlation_parameters, n_samples)
HF_to_air_incin, correlation_distributions, correlation_parameters = lhs.lhs_distribution(design_assumptions.loc['HF_to_air_incin'], correlation_distributions, correlation_parameters, n_samples)
Cl_to_air_incin, correlation_distributions, correlation_parameters = lhs.lhs_distribution(design_assumptions.loc['Cl_to_air_incin'], correlation_distributions, correlation_parameters, n_samples)
TFA_to_air_incin, correlation_distributions, correlation_parameters = lhs.lhs_distribution(design_assumptions.loc['TFA_to_air_incin'], correlation_distributions, correlation_parameters, n_samples)
...
# remove non PCTFE fields so they cannot appear in sensitivity
drop_keys = [
    'mass_fraction_PTFE', 'mass_fraction_PVDF',
    'rho_ptfe', 'rho_pvdf',
    'CO2_to_air_incin_PVDF', 'HF_to_air_incin_PVDF',
    'C2F6_to_air_incin', 'CF4_to_air_incin',
    'CO2_fossil_to_air_incin',  # not used directly in PCTFE emissions block
    'CO_to_air_incin'           # not used in PCTFE emissions block
]
for k in drop_keys:
...
PCTFE_mass = total_mass_input_waste * mass_fraction_PCTFE
mass_fraction_metal = 1 - mass_fraction_PCTFE
...
pctfe_vol_m3_d = (PCTFE_mass * 1000.0) / rho_pctfe
metal_vol_m3_d = (metal_mass * 1000.0) / rho_metal
solids_volume_disposal_PCTFE = pctfe_vol_m3_d + metal_vol_m3_d

driver_wages = driver_wages_US
diesel_cost = diesel_cost_US
landfill_distance = landfill_distance_US

solids_volume_disposal_incineration_PCTFE = solids_volume_disposal_PCTFE
solids_mass_disposal_incineration = solids_mass_disposal
solids_mass_total_disposal_incineration = solids_mass_total_disposal

# incineration costs for PCTFE
incineration_construction_cost_PCTFE = (3e6 * (solids_volume_disposal_incineration_PCTFE * normalize_factor) ** 0.271) / normalize_factor
...
incin_capital_cost_annualized_PCTFE = incineration_construction_cost_PCTFE * ((discount_rate * (1 + discount_rate) ** analysis_period) / ((1 + discount_rate) ** analysis_period - 1))
...
capital_cost_PCTFE = incin_capital_cost_annualized_PCTFE / (total_mass_input_waste * 365)
OM_cost_PCTFE = (incin_maintenance_PCTFE + incin_material_PCTFE + transport_solids_incineration_annual_cost + fuel_cost_annual) / (total_mass_input_waste * 365)
energy_cost_PCTFE = incin_energy_cost_PCTFE / (total_mass_input_waste * 365)
total_cost_PCTFE = capital_cost_PCTFE + OM_cost_PCTFE + energy_cost_PCTFE
...
# PCTFE direct emissions to air
direct_emissions_from_waste_PCTFE = (
  ...
  + solids_mass_disposal_incineration * mass_fraction_PCTFE * pollution_control_factor * CO2_to_air_incin_PCTFE * CO2_fossil_to_air_LCA
  + solids_mass_disposal_incineration * mass_fraction_PCTFE * pollution_control_factor * HF_to_air_incin * HF_to_air_LCA
  + solids_mass_disposal_incineration * mass_fraction_PCTFE * pollution_control_factor * Cl_to_air_incin * Cl_to_air_LCA
  + solids_mass_disposal_incineration * mass_fraction_PCTFE * pollution_control_factor * TFA_to_air_incin * TFA_to_air_LCA
) * 365

total_impacts_direct_emissions_from_waste_PCTFE = direct_emissions_from_waste_PCTFE / (total_mass_input_waste * 365)
total_impacts_PCTFE = total_impacts_capital + total_impacts_OM + total_impacts_energy + total_impacts_direct_emissions_from_waste_PCTFE

# results for PCTFE
impact_cols = ['AP','GWP','EFW','EP','HTC','HTNC','ODP','PMFP','MIR']
gwp_PCTFE = np.ravel(np.asarray(total_impacts_PCTFE)[:, 1])

df_results_PCTFE = pd.DataFrame({
    'cost_incineration_PCTFE': np.ravel(total_cost_PCTFE),
    'impacts_GWP_incineration_PCTFE': gwp_PCTFE
})

df_impacts_PCTFE = pd.DataFrame(total_impacts_PCTFE, columns=impact_cols)
df_direct_emissions_impacts_PCTFE = pd.DataFrame(total_impacts_direct_emissions_from_waste_PCTFE, columns=impact_cols)
...
df_cost = pd.DataFrame({
    'capital_cost_PCTFE': np.ravel(capital_cost_PCTFE),
    'OM_cost_PCTFE': np.ravel(OM_cost_PCTFE),
    'energy_cost_PCTFE': np.ravel(energy_cost_PCTFE),
    'total_cost_PCTFE': np.ravel(total_cost_PCTFE),
})

# sensitivity inputs dataframe
dfinputs = pd.DataFrame(inputs_for_sensitivity)[input_name_order]

# PCTFE sensitivity using only PCTFE impacts
df_impacts = df_impacts_PCTFE.copy()
sensitivity_cost_PCTFE = dfinputs.corrwith(df_cost['total_cost_PCTFE'], method='spearman')
sensitivity_PCTFE = pd.DataFrame({
    'cost': sensitivity_cost_PCTFE,
...
with pd.ExcelWriter("trial_PCTFE.xlsx") as writer:
    ...
    df_direct_emissions_impacts_PCTFE.to_excel(writer, sheet_name="direct_emissions_impacts_PCTFE", index=False)
    df_impacts_PCTFE.to_excel(writer, sheet_name="impacts_PCTFE", index=False)

    df_cost.to_excel(writer, sheet_name="cost", index=False)
    sensitivity_PCTFE.to_excel(writer, sheet_name="sensitivity_PCTFE")
